...
def detect_domain(tokens: Iterable[Token], config: CompressionConfig) -> DomainDetection:
    ...
    lowered = [tok.lower() for tok in tokens_list]
    total = len(lowered)

    python_keywords = {"def", "class", "import", "return", "for", "if", "elif", "else", "self"}
    json_tokens = {"{", "}", "[", "]", ":", ","}
    policy_tokens = {"allow", "deny", "policy", "rule", "effect", "resource", "action", "user"}
    ...
    if python_hits >= 2 and python_score >= 0.08:
        return DomainDetection(domain="code-python", confidence=min(1.0, python_score + 0.25))
    ...
    if policy_hits >= 3 and policy_score >= 0.1:
        return DomainDetection(domain="security-policy", confidence=min(1.0, policy_score + 0.2))
